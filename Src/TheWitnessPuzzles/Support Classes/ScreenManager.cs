...
namespace GameManager
{
    public class ScreenManager
    {
        ...
        public Dictionary<string, Texture2D> TextureProvider { get; private set; } 
            = new Dictionary<string, Texture2D>();
        public Dictionary<string, SpriteFont> FontProvider { get; private set; } 
        ...
        System.Text.RegularExpressions.Regex texIsMultiple =
            new System.Text.RegularExpressions.Regex(@"\d+\.\.\d+");

        System.Text.RegularExpressions.Regex texMultipleGetName = 
            new System.Text.RegularExpressions.Regex(@".+(?=\d+\.\.)");

        FadeTransition transitionAnimation = new FadeTransition(15, 20, 10);
        Texture2D texPixel;

        public void AddScreen<TScreen>(bool replaceCurrent = false, 
            bool doFadeAnimation = false, params object[] data) 
            where TScreen : GameScreen => AddScreen<TScreen>(replaceCurrent 
                ? 1 : 0, doFadeAnimation, data);
        public void AddScreen<TScreen>(int popNScreensFromStack, 
            bool doFadeAnimation = false, params object[] data)
        ...
        private void _addScreen<TScreen>(
            int popNScreensFromStack, params object[] data) where TScreen : GameScreen
        {
            GameScreen screen =
                (TScreen) Activator.CreateInstance(typeof(TScreen), ScreenSize, 
                Device, TextureProvider, FontProvider, Content, data);
        ...
        public void Initialize(Game1 game, GraphicsDevice device, ContentManager contentManager)
        {
            TheGameWindow = game;
            Device = device;
        ...
        public void LoadContent()
        {
            ...
            foreach (string texName in texturesToLoad)
            {
                ...
                else
                {
                    ...
                    string[] b = texIsMultiple.Match(texName).Value.Split(new string[] { ".." }, 
                        StringSplitOptions.RemoveEmptyEntries);

                    //int[] range = Array.ConvertAll(b, int.Parse);
                    int[] range = b.Select(s => Int32.Parse(s)).ToArray();
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (transitionAnimation.IsActive)
                spriteBatch.Draw(texPixel, new Rectangle(Point.Zero, ScreenSize), 
                    Color.Black * transitionAnimation.Opacity);
        }
        ...
        {
            TheGameWindow.SetFullscreen(SettingsManager.IsFullscreen);
        }
    }
}
