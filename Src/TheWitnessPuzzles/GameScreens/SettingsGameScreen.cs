...
namespace GameManager
{
    public class SettingsGameScreen : GameScreen
    {
        ...
        private void SpawnButtons()
        {
            ...
            ToggleButton btnFullscreen = new ToggleButton(new Rectangle(), texCheckbox[1], 
                texCheckbox[0], null, SettingsManager.IsFullscreen);

            btnFullscreen.Click += () => {
                SettingsManager.IsFullscreen = btnFullscreen.IsActivated;
                Game1._screenManager.UpdateFullscreen();
                SoundManager.PlayOnce(SettingsManager.IsFullscreen 
                    ? Sound.MenuTick : Sound.MenuUntick);
            };
            btnFullscreen.UpdateButton += () => {
                btnFullscreen.SetPositionAndSize(buttonsArea.Location
                    + new Point(buttonsArea.Width - buttonHeight, (int)(buttonHeight * btnHeightMod * 3)), new Point(buttonHeight, buttonHeight));
            };

            buttons.Add(btnFullscreen);

            ToggleButton btnBloomFX = new ToggleButton(new Rectangle(), 
                texCheckbox[1], texCheckbox[0], null, SettingsManager.BloomFX);
            ...
            btnBloomFX.UpdateButton += () => {
                btnBloomFX.SetPositionAndSize(buttonsArea.Location 
                    + new Point(buttonsArea.Width - buttonHeight, (int)(buttonHeight * btnHeightMod * 4)), new Point(buttonHeight, buttonHeight));
            };
            ...
            btnGeneratorSettings.Click += () => {
                Game1._screenManager.AddScreen<ExtraSettingsGameScreen>(false, true);
            ...
            btnBack.Click += () => {
                Game1._screenManager.GoBack();
                SoundManager.PlayOnce(Sound.MenuEscape);
            };
            btnBack.UpdateButton += () => {
                btnBack.SetPositionAndSize(new Point(buttonsArea.X,
                    ScreenSize.Y - buttonHeight * 2), new Point(buttonsArea.Width, (int)(buttonHeight * 1.5f)));
            };
            buttons.Add(btnBack);
        ...
        private void RenderBackgroundToTexture()
        {
            using (SpriteBatch spriteBatch = new SpriteBatch(GraphicsDevice))
            {
                ...
                Vector2 captionPosition =
                    new Vector2(ScreenSize.X / 2 - captionSize.X / 2, 
                    ScreenSize.Y * 0.05f);

                spriteBatch.DrawString(font, caption, captionPosition, Color.White, 
